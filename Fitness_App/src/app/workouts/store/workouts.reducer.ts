...
import { addExercise, addWorkout, removeExercise, removeWorkout, setWorkouts, startWorkoutUpdate, stopWorkoutUpdate, storeWorkouts, updateWorkout } from "./workouts.actions";
...
export const workoutReducer = createReducer(
    ...
    on(removeWorkout, (entries, payload) => {
        const entriesClone: WorkoutState = JSON.parse(JSON.stringify(entries));
        entriesClone.Workouts.splice(payload.index, 1);
        return entriesClone;
    }),
    ...
    on(startWorkoutUpdate, (entries, payload) => {
        const entriesClone: WorkoutState = JSON.parse(JSON.stringify(entries));
        entriesClone.editedWorkoutIndex = payload.index;
        entriesClone.editedWorkout = entriesClone.Workouts[payload.index];
        return entriesClone;
    }),
    on(stopWorkoutUpdate, (entries) => {
        const entriesClone: WorkoutState = JSON.parse(JSON.stringify(entries));
        entriesClone.editedWorkoutIndex = -1;
        entriesClone.editedWorkout = null;
        return entriesClone;
    }),
    on(addExercise, (entries, payload) => {
        const entriesClone: WorkoutState = JSON.parse(JSON.stringify(entries));
        console.log(entriesClone);
        entriesClone.Workouts[entriesClone.editedWorkoutIndex].exercise.unshift(payload);
        return entriesClone;
    }),
    on(removeExercise, (entries, payload) => {
        const entriesClone: WorkoutState = JSON.parse(JSON.stringify(entries));
        console.log(entriesClone.editedWorkoutIndex);
        entriesClone.Workouts[entriesClone.editedWorkoutIndex].exercise.splice(payload.index, 1);
        return entriesClone;
    }),
