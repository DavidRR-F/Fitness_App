...
export class WorkoutSubmitComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.params.subscribe(
      (params: Params) => {
        ...
        if(!this.editMode){
          this.workoutsService.onAddWorkout(new Workout('New Workout', 
          [new Exercise('New Exercise', this.defaultImg,0,0,0)]))
          this.index = +this.workoutsService.getNewWorkout() - 1;
        }
        this.workoutsService.onUpdateStart(this.index);
        this.workout = this.workoutsService.onWorkoutSub(this.index);
        this.workoutForm = this.workoutsService.onWorkoutForm(this.index);
  ...
  ngOnDestroy(): void {
    this.workoutsService.onUpdateStop();
  }
  ...
  onSubmit() {
    this.saved = true;
    this.workoutsService.onUpdateWorkout(this.workoutForm.value);
  }

  onAddExercise(exercise: Exercise){
    this.workoutsService.onUpdateWorkout(this.workoutForm.value);
    this.controls.push(this.workoutsService.createExerciseFormGroup());
    if (exercise){
      this.workoutsService.onAddExercise(exercise);
    } else {
      this.workoutsService.onAddExercise(new Exercise('', this.defaultImg, 0, 0, 0));
    }
  }
  ...
  onDelete(exerciseIndex: number) {
    this.workoutsService.onDeleteExercise(exerciseIndex);
    this.controls.splice(exerciseIndex, 1);
  }

  get controls() { // a getter!
    return (<FormArray>this.workoutForm.get('exercise')).controls;
  }
